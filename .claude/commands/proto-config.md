---
description: Show or change your ProtoPeek setup — token status, default reviewer domain, prototype log.
argument-hint: [set-default <domain> | clear-default]
allowed-tools: Bash
---
...
Arguments: `$ARGUMENTS` — empty to show, `set-default <domain>` or `clear-default` to change.

Steps:
1. **Load config** (global store) — same as `/proto-up` step 1. If there is no config,
   say so and point at `/proto-up` (which walks through setup); nothing to show or set.
2. **No arguments → show the setup:**
...
3. **`set-default <domain>`** → rewrite the `PROTOPEEK_DEFAULT_DOMAIN=` line in
   `$CFG/config` (atomic write — temp file + rename; keep chmod 600). Strip any `@`.
   **`clear-default`** → set it to empty the same way.
   Confirm what changed and note it affects **future uploads only** — existing
   prototypes keep their rules (edit those with `/proto-up --allow` on an update, or
